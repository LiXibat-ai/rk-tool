...
/*
FILE *fp: 指向要读取文件的文件指针。
char *buffer: 用来存储读取内容的缓冲区指针（但在函数内实际是动态分配的）。
int *len: 用来返回读取行的长度。
*/
...
/*
一开始为 buffer 分配初始内存。每次读取一个字符并存储在 buffer 中时，
如果 buffer 不够大，则使用 realloc 函数重新分配内存。
*/
    while ((ch = fgetc(fp)) != '\n' && ch != EOF)
    /*
    使用 fgetc(fp) 函数逐字符读取，直到读取到换行符 \n 或文件结束 EOF。
    */
    {
        buff_len++;
    ...
    *len = buff_len;
    ...
    /*
    如果遇到 EOF 并且没有读取到任何内容，释放内存并返回 NULL。
    否则，返回已分配和填充的 buffer。
    */
    return buffer;
}
...
char *lines[]: 用来存储每一行内容的字符串数组。
int max_line: 要读取的最大行数。
*/
static int readLines(const char *fileName, char *lines[], int max_line)
{
    FILE *file = fopen(fileName, "r");
    // 打开文件：使用 fopen 打开文件。如果文件无法打开，打印错误信息并返回 -1。
    char *s;
    int i = 0;
    int n = 0;

    if (file == NULL) // 判断文件指针是否为空
    {
        printf("Open %s fail!\n", fileName);
        return -1;
    }
    ...
    // 循环调用 readLine 函数读取文件中的每一行，并将每一行的内容存储在 lines 数组中。
    // 如果文件中的行数超过了 max_line，则提前终止读取。
    fclose(file);
    return i;
}
